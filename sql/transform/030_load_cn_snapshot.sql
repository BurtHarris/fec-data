-- Load per-cycle row counts for candidate master files from staging.
-- Reads all available cycle files at data/{cycle}/staging/cn.txt.

CREATE OR REPLACE TEMP TABLE _cn_counts AS
SELECT
    CAST(NULLIF(regexp_extract(replace(filename, chr(92), '/'), '(^|/)data/([0-9]{4})/staging/cn\.txt$', 2), '') AS INTEGER) AS cycle,
    count(*) AS row_count,
