...
# Usage:
#   # Run directly
#   .\scripts\probe-fec-2026.ps1
#
#   # Run as a trackable background job
#   $job = Start-Job -FilePath .\scripts\probe-fec-2026.ps1
...
foreach ($fileInfo in $files) {
    $url = "$baseUrl/$($fileInfo.Name).zip"
    Write-Output "[CHECK] $($fileInfo.Name).zip  ($($fileInfo.Desc))"

    try {
        $response = Invoke-WebRequest -Uri $url -Method Head -UseBasicParsing -TimeoutSec 15
        $size = $response.Headers['Content-Length'] | Select-Object -First 1
        $etag = $response.Headers['ETag'] | Select-Object -First 1
        $modified = $response.Headers['Last-Modified'] | Select-Object -First 1
